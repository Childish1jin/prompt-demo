...
角色
# https://github.com/giscafer/cursor-claude-4-demo/blob/master/Prompt.md
...
规范的间距
UI设计规格
单个设计图尺寸为 375x812PX，带有mock up模拟手机的样式
图标：引用在线矢量图标库内的图标
图片：使用Unsplash开源图片网站链接的形式引入
样式必须使用<link>标签引入 tailwindcss CDN来完成
状态栏显示时间、信号等模拟信息
信号栏与标题栏背景色必须一致（可以都是透明）
底部tab栏必须为白色填充，100%不透明度
使用固定高度防止容器变形
注意事项
mock up的颜色只能使用黑色
所有html中都强制隐藏滚动条
